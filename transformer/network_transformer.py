...
from transformer import CompressionTransformer
...
class Net(nn.Module):
    ...
    def __init__(self, min_accuracy, colors, depth, model_size, n, L, cuda_backend, pareto_interval, pareto_alpha):
        ...
        # transformer part
        self.transformer = CompressionTransformer(model_size, z_centroids=self.c)
    ...
    def transforward(self, x, training=False):
        ...
        # todo: solve eval mode problem (BN shit)

        output = self.transformer(z_k)
        output.update({'z': z_k})
