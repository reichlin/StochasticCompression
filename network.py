...
class Net(nn.Module):
    ...
    def __init__(self, min_accuracy, colors, depth, model_size, n, L, compression_sampling_function, adaptive_compression_sampling, k_sampling_policy, exploration_epsilon, a_size, a_depth, a_act, decoder_type):
    ...
    def E(self, x):
        ...
        if self.adaptive_compression_sampling:
            ...
            alfa = self.Alpha(a)  # [B, 3, H, W]
            alfa = torch.sigmoid(alfa)

            if self.compression_sampling_function == 1:
                alpha = alfa[:,0,:,:] * (2. - 0.01) + 0.01
                l = alfa[:,1,:,:] * (float(self.n) - 1.) + 1.
                h = None
            elif self.compression_sampling_function == 2:
                alpha = alfa[:,0,:,:] * (2. - 0.01) + 0.01
                l = alfa[:,1,:,:] * (float(self.n) - 2.) + 2.
                h = alfa[:,2,:,:] * (float(self.n) - (l+2)) + (l+2)
    ...
    def sample_compression(self, mu, alpha, l, h):
        ...
        if self.adaptive_compression_sampling:
            ...
            if self.compression_sampling_function == 0:
                k_compression_limit = torch.cuda.FloatTensor(mu.shape).uniform_() * torch.clamp(mu + 0.02, 0, 1.0)
                ...
                l = torch.clamp(torch.ceil(mu - mu/2.), 1.0)
    ...
    def forward(self, x, training=True):
        ...
        return x_hat_compress, x_hat_k, log_pk, log_p_compression, k, k_compression, mu, a, z, quantization_error
    ...
    def get_accuracy(self, x):
        ...
        _, x_hat, _, _, _, _, mu, _, z, _ = self.forward(x, False)
